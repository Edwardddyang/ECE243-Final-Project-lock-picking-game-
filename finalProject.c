              ...
              if (!pinSet[currentPinIndex]) {
                ...
                if (pinYPositions[currentPinIndex] >= SHEAR_LINE_Y - margin &&
                  ...
                  if (currentPinIndex == pinSequence[currentSequenceIndex]) {
                    // SUCCESS! They hit the line AND it's the right pin.
                    pinSet[currentPinIndex] = true;
                    currentSequenceIndex++;
                    triggerSuccessSound();
                  } else {
                    // SNAP! They hit the line, but it's the WRONG PIN!
                    for (int i = 0; i < NUM_PINS; i++) {
                      pinSet[i] = false;
                    }
                    currentSequenceIndex = 0;
                  ...
                  // SNAP! They completely MISSED the red line!
                  // Punishment: Drop everything.
                  for (int i = 0; i < NUM_PINS; i++) {
                    pinSet[i] = false;
                  }
                  currentSequenceIndex = 0;
